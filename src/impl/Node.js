    ...
    Node.prototype = Object.create(Object.prototype, {
        ...
        dispatchEvent: constant(function dispatchEvents(event) {
            ...
            function invoke(target, event) {
                ...
                if (event.eventPhase === AT_TARGET) {
                // XXX: if we're AT_TARGET, then we need to do 
                // both regular and capturing listeners.  And we need
                // to invoke them in the order in which they were registered
                // This means that I can't maintain two separate lists.
                // Each listeners has to have a capturing flag.
                // Which also means that I can treat function objects
                // specially and skip the typeof test below...
                // So I have to go and rewrite the registration
                // de-registration methods above to use a single list.
                }
                else {
                    ...
                    if (!listeners) return;  // No listeners for this type
                    
                    invokeList(listeners, event)
                }

                function invokeList(listeners, event) {
            ...
            if (event.bubbles && !event._stopPropagation) {
                ...
                for(let i = 0, n = ancestors.length; i < n; i++) {
                    invoke(ancestors[i], event, false);
                    if (event._stopPropagation) break;
                }
            }
